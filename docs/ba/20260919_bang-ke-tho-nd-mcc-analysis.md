...
5. Tạo ra 8 sheets nghiệp vụ kết quả được gắn thêm vào workbook gốc:
...
| BR-03 | **Khách hàng Mapping:** Tra cứu bảng `customers` theo cặp `(ten_khach_hang, dia_chi_giao_hang)` (chuẩn hóa trim, lowercase). Ưu tiên rule khớp theo `supplier_code` cho MCC/NDFC khi slot = `'UNI 1'`. Lấy về:<br>- Đại lý (`diem_tra_hang`)<br>- Điểm giao hàng thực tế (`tuyen_phuong`)<br>- Điểm giao hàng tính phí (`diem_giao_hang_tinh_phi`). |
...
| BR-06 | **Cấu trúc Sub-sheet Tóm tắt (CLV, UNI, TT):** Thu gọn theo từng dòng Hóa đơn (1 dòng / hóa đơn), tổng hợp số liệu trọng lượng hóa đơn và trọng lượng chuyến tương ứng. |
| BR-07 | **Toàn vẹn Workbook Output & Thứ tự Sheets:** File output xuất 11 sheets theo đúng thứ tự chuẩn:<br>1. `NCC`<br>2. `Sheet1`<br>3. `Processed`<br>4. `MCC (goc)`<br>5. `MCC-clv`<br>6. `MCC (uni)`<br>7. `MCC (tt)`<br>8. `NDFC (goc)`<br>9. `NDFC-clv`<br>10. `NDFC (uni)`<br>11. `NDFC (tt)`<br>- Các sheets cơ sở `NCC`, `Sheet1`, `Processed` được giữ lại từ file input (nếu `NCC`/`Sheet1` chưa có sẽ được khởi tạo, giữ nguyên dữ liệu đã có).<br>- Các sheets ngoài phạm vi (như `VFM`, `CLV`, `STHI`, `Process 1-8`, `Sheet31-8`, v.v.) được loại bỏ khỏi output. |
