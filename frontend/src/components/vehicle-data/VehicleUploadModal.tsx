...
import { useUploadVehicles } from '../../hooks/useVehicles';
import type { UploadVehicleRow, UploadVehicleError } from '../../api/vehicleApi';
...
function generateTemplate(): void {
  ...
  const ws = XLSX.utils.aoa_to_sheet([['Biển số', 'Loại', 'Tài xế']]);
  XLSX.utils.book_append_sheet(wb, ws, 'DuLieuXe');
  XLSX.writeFile(wb, 'template_du_lieu_xe.xlsx');
}

export function VehicleUploadModal({ isOpen, onClose, onSuccess }: Props) {
  const uploadMutation = useUploadVehicles();
  const [file, setFile] = useState<File | null>(null);
  const [fileError, setFileError] = useState('');
  const [uploadErrors, setUploadErrors] = useState<UploadVehicleError[]>([]);
  ...
  const parseExcel = (f: File): Promise<UploadVehicleRow[]> => {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          const parsed: UploadVehicleRow[] = rows
            .filter((row) => {
              const bienSo = String(row['Biển số'] ?? '').trim();
              return bienSo !== '' && bienSo !== 'null';
            })
            .map((row) => {
              const taiXeRaw = row['Tài xế'];
              const taiXe: string[] =
                taiXeRaw != null && String(taiXeRaw).trim() !== '' && String(taiXeRaw).trim() !== 'null'
                  ? String(taiXeRaw)
                      .split(',')
                      .map((s) => s.trim())
                      .filter(Boolean)
                  : [];
              return {
                bien_so: String(row['Biển số'] ?? '').trim(),
                loai: String(row['Loại'] ?? '').trim(),
                tai_xe: taiXe,
              };
            });
  ...
  const handleUpload = async () => {
    ...
    if (rows.length === 0) {
      setFileError('File không có dữ liệu hợp lệ. Kiểm tra cột "Biển số".');
      return;
    }

    try {
      const result = await uploadMutation.mutateAsync(rows);
      onSuccess(`Đã upload ${result.inserted} xe thành công!`);
      handleClose();
    } catch (err: unknown) {
      if (err && typeof err === 'object' && 'response' in err) {
        const e = err as {
          response?: { data?: { data?: { errors?: UploadVehicleError[] }; message?: string }; status?: number };
        };
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Upload Dữ liệu xe từ Excel" size="lg">
                ...
                <tbody>
                  {uploadErrors.map((err, i) => (
                    <tr key={i} className="border-t border-red-100 dark:border-red-900/40">
                      ...
                      <td className="px-3 py-2 font-mono text-neutral-800 dark:text-neutral-200">{err.bien_so}</td>
