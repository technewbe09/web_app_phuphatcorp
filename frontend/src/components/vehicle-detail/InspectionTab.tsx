...
export function InspectionTab({ vehicleId, setToasts }: Props) {
  const [modal, setModal] = useState<{ type: 'create' } | { type: 'edit'; record: InspectionRecord } | { type: 'view'; id: number } | null>(null);
  const [page, setPage] = useState(1);
  const PAGE_SIZE = 20;

  const { data, isLoading, isError, refetch } = useGetInspections({ vehicle_id: vehicleId, page, limit: PAGE_SIZE });
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-3 text-neutral-500 dark:text-neutral-400">
              <p className="text-sm">Chưa có dữ liệu đăng kiểm.</p>
                    ...
                    return (
                      <TableRow key={record.id}>
                        ...
                        <TableCell className="text-neutral-700 dark:text-neutral-300 text-sm">
                          {new Date(record.inspection_date).toLocaleDateString('vi-VN')}
                        </TableCell>
      ...
      <InspectionFormModal
        ...
        inspection={modal?.type === 'edit' ? modal.record : null}
        viewInspectionId={modal?.type === 'view' ? modal.id : undefined}
