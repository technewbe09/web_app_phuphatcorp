...
export function RepairTab({ vehicleId, setToasts }: Props) {
  const [modal, setModal] = useState<{ type: 'create' } | { type: 'edit'; id: number } | { type: 'view'; id: number } | null>(null);
  const [page, setPage] = useState(1);
  const PAGE_SIZE = 20;

  const { data, isLoading, isError, refetch } = useGetVehicleRepairs(vehicleId, { page, limit: PAGE_SIZE });
  ...
  const handleDelete = async (id: number) => {
    if (!window.confirm('Bạn có chắc muốn xóa bản ghi sửa xe này?')) return;
    try {
      await deleteMutation.mutateAsync(id);
      showToast('Đã xóa bản ghi sửa xe');
    } catch {
      showToast('Không thể xóa bản ghi', 'error');
    }
  };

  const records = data?.repairs ?? [];
  ...
  return (
    <div className="space-y-4">
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-3 text-neutral-500 dark:text-neutral-400">
              ...
              <p className="text-sm">Không thể tải dữ liệu sửa xe.</p>
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-3 text-neutral-500 dark:text-neutral-400">
              <p className="text-sm">Chưa có lịch sử sửa xe.</p>
                  ...
                  {records.map((record, idx) => (
                    <TableRow key={record.id}>
                      ...
                      <TableCell className="text-neutral-700 dark:text-neutral-300 text-sm">
                        {new Date(record.repair_date).toLocaleDateString('vi-VN')}
                      </TableCell>
                      ...
                      <TableCell className="text-neutral-600 dark:text-neutral-400 text-sm">
                        {record.items && record.items.length > 0
                          ? `${record.items[0].item_name}${record.items.length > 1 ? ` (+${record.items.length - 1})` : ''}`
                          : '-'}
                      </TableCell>
                      ...
                      <TableCell className="text-center">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          <button
                            onClick={() => setModal({ type: 'edit', id: record.id })}
      ...
      <RepairFormModal
        ...
        repairId={modal?.type === 'edit' ? modal.id : modal?.type === 'view' ? modal.id : undefined}
