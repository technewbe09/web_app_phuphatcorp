...
import { useGetWeightAdjustments, useDeleteWeightAdjustment } from '../../../hooks/useWeightAdjustments';
import { WeightAdjustmentFormModal } from '../../../components/accounting-data/WeightAdjustmentFormModal';
import { WeightAdjustmentUploadModal } from '../../../components/accounting-data/WeightAdjustmentUploadModal';
import { useAuth } from '../../../hooks/useAuth';
import type { WeightAdjustment } from '../../../api/weightAdjustmentApi';

type ModalState =
  | { type: 'create' }
  | { type: 'edit'; row: WeightAdjustment }
  | { type: 'delete'; row: WeightAdjustment }
  | { type: 'upload' }
  | null;
...
export function WeightAdjustmentPage() {
  const { data, isLoading, isError, refetch } = useGetWeightAdjustments();
  const deleteWeightAdjustment = useDeleteWeightAdjustment();
  ...
  const filteredRows = useMemo(() => {
    ...
    return data.filter(
      (row) =>
        row.ma_hang.toLowerCase().includes(q) ||
  ...
  const handleDelete = async () => {
    ...
    setDeleteError('');
    try {
      await deleteWeightAdjustment.mutateAsync(modal.row.id);
          ...
          {isLoading ? (
            ...
            <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-32">Mã hàng hóa</TableHead>
                    <TableHead>Tên hàng hóa</TableHead>
                    <TableHead className="w-36 text-right">Giá trị cũ</TableHead>
                    <TableHead className="w-36 text-right">Giá trị điều chỉnh</TableHead>
                    {canManage && <TableHead className="w-24">Thao tác</TableHead>}
                  </TableRow>
                </TableHeader>
                <TableBody>
                    ...
                    return (
                      <TableRow key={row.id}>
                        ...
                        <TableCell className="text-right text-neutral-600 dark:text-neutral-400 tabular-nums">
                          {formatNumber(row.gia_tri_cu)}
                        </TableCell>
                        <TableCell className="text-right text-neutral-900 dark:text-neutral-100 tabular-nums font-medium">
                          {formatNumber(row.gia_tri_dieu_chinh)}
                        </TableCell>
                        {canManage && (
                          <TableCell>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <p className="text-sm text-neutral-700 dark:text-neutral-300">
              Bạn có chắc muốn xóa bản ghi Mã hàng:{' '}
              <span className="font-semibold">
                "{modal?.type === 'delete' ? modal.row.ma_hang : ''}"
              </span>
              ?{' '}
              <br />
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button
              variant="danger"
              isLoading={deleteWeightAdjustment.isPending}
