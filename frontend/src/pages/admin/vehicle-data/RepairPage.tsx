...
import { useGetRepairSummary } from '../../../hooks/useVehicleRepairs';
...
export function RepairPage() {
  const [modal, setModal] = useState<
    ...
    { type: 'history'; vehicle: VehicleRepairSummary } |
  ...
  const { data, isLoading, isError, refetch } = useGetRepairSummary({
  ...
  return (
    <div className="p-6 space-y-6">
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="w-32">Sửa gần nhất</TableHead>
                    <TableHead>Gara gần nhất</TableHead>
                    <TableHead className="w-40 text-right">Tổng tiền đã sửa</TableHead>
                    <TableHead className="w-20 text-center">Số lần sửa</TableHead>
                  ...
                  {vehicleList.map((v, idx) => (
                    <TableRow key={v.vehicle_id}>
                      ...
                      <TableCell className="text-neutral-700 dark:text-neutral-300 text-sm">
                        {v.latest_repair_date
                          ? new Date(v.latest_repair_date).toLocaleDateString('vi-VN')
                          : '-'}
                      </TableCell>
                      <TableCell className="text-neutral-700 dark:text-neutral-300 text-sm">
                        {v.latest_garage_name || '-'}
                      </TableCell>
                      <TableCell className="text-neutral-700 dark:text-neutral-300 text-sm text-right font-medium">
                        {formatCurrency(parseInt(v.total_repair_amount || '0', 10))}
                      </TableCell>
      ...
      {modal?.type === 'view' && modal.vehicle.latest_repair_id && (
        <RepairFormModal
          isOpen={true}
          onClose={() => setModal(null)}
          onSuccess={(msg) => showToast(msg)}
          onError={(msg) => showToast(msg, 'error')}
          repairId={modal.vehicle.latest_repair_id}
          viewMode
        />
      )}
