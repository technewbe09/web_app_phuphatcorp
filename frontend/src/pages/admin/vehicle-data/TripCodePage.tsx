...
import { useGetTripCodes, useDeleteTripCode } from '../../../hooks/useTripCodes';
import { TripCodeFormModal } from '../../../components/vehicle-data/TripCodeFormModal';
import { TripCodeUploadModal } from '../../../components/vehicle-data/TripCodeUploadModal';
import { formatCurrency, formatDateTime } from '../../../utils/format';
import type { TripCode } from '../../../api/tripCodeApi';

type ModalState =
  | { type: 'create' }
  | { type: 'edit'; row: TripCode }
  | { type: 'delete'; row: TripCode }
  | { type: 'upload' }
  | null;
...
export function TripCodePage() {
  const { data, isLoading, isError, refetch } = useGetTripCodes();
  const deleteTripCode = useDeleteTripCode();
  ...
  const filteredRows = useMemo(() => {
    ...
    return data.filter(
      (row) =>
        ...
        row.tuyen.toLowerCase().includes(q),
    );
  }, [data, search]);
  ...
  const handleDelete = async () => {
    ...
    setDeleteError('');
    try {
      await deleteTripCode.mutateAsync(modal.row.id);
      showToast('Đã xóa mã chuyến!');
      closeModal();
    } catch (err: unknown) {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-neutral-900 dark:text-neutral-100">Mã chuyến</h1>
      ...
      <Card>
        <CardContent className="p-4">
          <Input
            placeholder="Tìm theo Mã hoặc Tuyến..."
            value={search}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-3 text-neutral-500 dark:text-neutral-400">
              <p className="text-sm">
                {search
                  ? 'Không tìm thấy mã chuyến nào phù hợp.'
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="w-24 text-right">Số lượt</TableHead>
                    <TableHead className="w-24">Bốc xếp</TableHead>
                ...
                <TableBody>
                  {filteredRows.map((row) => (
                    <TableRow key={row.id}>
                      <TableCell className="font-medium text-neutral-900 dark:text-neutral-100">{row.ma}</TableCell>
                      <TableCell className="text-neutral-700 dark:text-neutral-300">{row.tuyen}</TableCell>
                      <TableCell className="text-right text-neutral-600 dark:text-neutral-400 tabular-nums">
                        {row.so_tien != null ? formatCurrency(row.so_tien) : '—'}
                      </TableCell>
                      <TableCell className="text-right text-neutral-600 dark:text-neutral-400 tabular-nums">
                        {row.so_luot}
                      </TableCell>
                      <TableCell>
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                          ...
                          {row.boc_xep === 'yes' ? 'Có' : 'Không'}
                        </span>
                      </TableCell>
                      <TableCell className="text-neutral-500 dark:text-neutral-400 max-w-[12rem] truncate" title={row.ghi_chu ?? undefined}>
                        {row.ghi_chu || '—'}
                      </TableCell>
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <p className="text-sm text-neutral-700 dark:text-neutral-300">
              ...
              <span className="font-semibold">
                "{modal?.type === 'delete' ? modal.row.ma : ''}"
              </span>
              ?{' '}
              <br />
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button
              variant="danger"
              isLoading={deleteTripCode.isPending}
