...
import { Upload, Trash2, AlertTriangle, RefreshCw } from 'lucide-react';
...
import { DeleteVehicleDialog } from '../../../components/catalog/DeleteVehicleDialog';
...
export function VehicleCatalogPage() {
  const [modal, setModal] = useState<
    { type: 'upload' } | { type: 'delete'; vehicle: Vehicle } | null
  >(null);
  ...
  const { data, isLoading, isError, refetch } = useGetVehicles(search, page, PAGE_SIZE);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <DeleteVehicleDialog
        ...
        onSuccess={(msg) => showToast(msg)}
        onError={(msg) => showToast(msg, 'error')}
        vehicle={modal?.type === 'delete' ? modal.vehicle : null}
      />
    </div>
  );
}
