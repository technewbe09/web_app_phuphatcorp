...
import { useGetInnerCityCustomers } from '../../../hooks/useInnerCityCustomers';
import { InnerCityCustomerFormModal } from '../../../components/catalog/InnerCityCustomerFormModal';
import { DeleteInnerCityCustomerDialog } from '../../../components/catalog/DeleteInnerCityCustomerDialog';
import type { InnerCityCustomer } from '../../../api/innerCityCustomerApi';

type ModalState =
  | { type: 'create' }
  | { type: 'edit'; customer: InnerCityCustomer }
  | { type: 'delete'; customer: InnerCityCustomer }
  | null;
...
export function InnerCityCustomerPage() {
  ...
  const { data, isLoading, isError, refetch } = useGetInnerCityCustomers(search, page, PAGE_SIZE);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <Card>
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-3 text-neutral-500 dark:text-neutral-400">
              <p className="text-sm">
                {search ? 'Không tìm thấy khách hàng nào phù hợp.' : 'Chưa có khách hàng nội thành nào.'}
              </p>
              {!search && (
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="w-48">Mã KH</TableHead>
                    <TableHead>Tên khách hàng</TableHead>
                    <TableHead className="w-64 hidden md:table-cell">Ghi chú</TableHead>
                ...
                <TableBody>
                  {customers.map((cust, idx) => (
                    <TableRow key={cust.id}>
                      ...
                      <TableCell className="text-neutral-500 dark:text-neutral-400 text-sm hidden md:table-cell truncate max-w-64">
                        {cust.notes || '—'}
                      </TableCell>
                      <TableCell className="text-neutral-500 dark:text-neutral-400 text-sm">
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() => setModal({ type: 'edit', customer: cust })}
                          ...
                          <button
                            onClick={() => setModal({ type: 'delete', customer: cust })}
      ...
      <DeleteInnerCityCustomerDialog
        isOpen={modal?.type === 'delete'}
        onClose={() => setModal(null)}
        onSuccess={(msg) => showToast(msg)}
        onError={(msg) => showToast(msg, 'error')}
        customer={modal?.type === 'delete' ? modal.customer : null}
      />
    </div>
  );
}
